def main():
    ...
    number_of_event = int(raw_input("Number of choosen people: "))

    probability_of_no_coincident = 1.0

    if number_of_event > 1:
        for i in range(2, number_of_event + 1):
            probability_of_no_coincident = \
                (1.0 - ((i - 1) / float(possible_outcome))) * probability_of_no_coincident
    
    output_format = "The probability of n choosen thing are difference = {}"
